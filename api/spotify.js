// api/spotify.js
// Vercel Serverless Function to dynamically render live Spotify playing status as SVG.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (nowPlaying && nowPlaying.item) {
      ...
      if (recentlyPlayed && recentlyPlayed.items && recentlyPlayed.items.length > 0) {
        ...
        trackData = {
          ...
          progress: lastTrack.duration_ms,
    ...
    console.error("Spotify API error, rendering fallback:", error.message);
    if (!client_id || !client_secret) {
      // Mock fallback for preview before Vercel Env variables are configured
      try {
        ...
        return res.status(200).send(generateOfflineSVG());
      }
    }
    return res.status(200).send(generateOfflineSVG());
  }
